...
export default function Home() {
  ...
  return <main className="home-page dashboard-page">
    ...
    <section className="dashboard-finance-hero" onClick={() => navigate("/finance")}>
      ...
      <div className="finance-hero-heading"><span className="dashboard-card-icon"><WalletCards size={21} /></span><div><small>收支概览</small>{monthEntries.length ? <h2 className="finance-hero-balance"><span>本月结余</span><strong aria-live="polite">{balanceVisible ? <>{balanceSign}{money(Math.abs(monthBalance))}</> : "****"}</strong><button type="button" className="finance-balance-toggle" aria-label={balanceVisible ? "隐藏结余金额" : "显示结余金额"} aria-pressed={balanceVisible} onClick={event => { event.stopPropagation(); setBalanceVisible(value => !value); }}>{balanceVisible ? <Eye size={18} /> : <EyeOff size={18} />}</button></h2> : <h2>本月还没有收支记录</h2>}<p>{monthEntries.length ? `本月已记录${monthEntries.length}笔 · 今日${todayEntries.length}笔` : "从记录每一笔开始，更从容地安排生活"}</p></div></div>
