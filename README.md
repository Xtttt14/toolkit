# 个人工具箱

个人工具箱是一款面向Windows的本地桌面应用，将今日仪表盘、饮水提醒、待办清单、番茄钟、记账助手和校园助手集中在同一个程序中。数据默认只保存在本机，无需注册账号或连接云服务。

## 功能

### 今日仪表盘

- 汇总今日待办、饮水、专注、收支、课程与近期考试
- 每张卡片均可进入对应功能页面
- 支持快速记录饮水、新建Todo、开始专注和记账

### 饮水提醒

- 按杯数或毫升查看今日进度
- 管理常用杯子和容量
- 设置工作时间、提醒间隔和重复提醒
- 补记饮水记录并查看日历、周统计和月统计
- 支持Windows系统通知

### 待办清单

- 使用P0—P3优先级管理任务
- 支持截止时间、提前提醒、描述和自定义标签
- 每个任务最多包含8个纵向子任务，并显示步骤编号、完成进度和清晰的完成状态
- 未完成与已完成事项分区显示；父任务与子任务完成状态自动联动
- 主任务和子任务均支持拖拽调整顺序，排序结果会保存在本地
- 支持搜索、筛选、排序、批量删除和完成状态管理
- 到期前通过Windows系统通知提醒

### 番茄钟

- 创建并保存可复用的专注任务，从任务选择页快速开始
- 倒计时支持30/60/90/120分钟预设和自定义时长，时间归零后自动完成
- 正计时无需设置计划时长，由用户手动标记完成
- 在新建或编辑任务时选择、新建和删除可复用标签
- 切换页面、最小化或隐藏到托盘后仍由后台准确计时
- 倒计时结束时发送Windows系统通知
- 提供落日、暮蓝、林间和自定义图片沉浸背景，以及光环、数码和极简3种时钟样式
- 统计累计和今日专注次数、时长及放弃次数
- 支持日、周、月和自定义范围的柱状图、曲线图、任务分布图与标签分布图
- 点击标签分布可下钻查看标签内的任务时长分布

### 记账助手

- 记录收入、支出、日期、标签和备注
- 支持按起止日期批量添加连续账单
- 内置常用收支标签，支持新增、改名、删除和拖拽排序
- 今日记账、月历和统计报表三个页面
- 支持周、月、年收支趋势及支出分类占比
- 支持JSON备份与恢复
- 提供“总计项目”页：为纸张、设备等长期开销建立累计项目，可直接新增记录或关联多笔每日账单；关联账单的修改和删除会自动同步

### 校园助手

- 按教学周查看日常课表并设置课程提醒
- 导入考试信息、查看考试日历并设置考试提醒
- 未来考试会自动创建P0级“考试”标签Todo，考试结束后自动完成

### 工具箱设置

- 在应用“文件”菜单或托盘菜单中打开独立的工具箱设置
- 配置关闭时是否询问，以及不询问时隐藏到托盘或退出程序
- 支持主窗口全局快捷键和Windows登录后自动启动
- 可控制删除、放弃等操作是否需要二次确认

## 技术栈

- Electron 33
- React 18
- React Router 6
- Vite 6
- electron-store 8
- lucide-react

## 开发

### 维护原则

- 修复缺陷时应先追溯共性根因及相关状态、数据流，再用通用规则或机制解决同一类问题。
- 不应只为当前一条消息、单条记录或截图场景添加特判；新增逻辑应能处理语义相同的后续输入。
- 涉及写入或删除时，应将候选识别、选择、确认与取消作为完整流程设计并验证。
- 每次缺陷修复的交付说明应包含问题理解（根因和影响范围）、修改方式（通用机制）及验证结果。

环境要求：

- Windows 10或Windows 11
- Node.js 18或更高版本
- npm

安装依赖：

```powershell
npm install
```

启动开发环境：

```powershell
npm run start
```

仅构建前端：

```powershell
npm run build
```

生成Windows安装版：

```powershell
npm run dist
```

安装版会在启动后自动检查 GitHub Releases 中的新版本。发现更新后，可以在“工具箱设置 → 软件更新”中下载，并重启完成更新；本地业务数据不会被覆盖。

### 飞书机器人（安装版）

安装版启动后，会从用户数据目录读取飞书和 DeepSeek 凭据。先完全退出工具箱，在以下目录新建 `.env`：

```text
%APPDATA%\personal-toolbox\.env
```

填写（不要提交或分享此文件）：

```env
FEISHU_APP_ID=你的飞书App_ID
FEISHU_APP_SECRET=你的飞书App_Secret
FEISHU_ALLOWED_OPEN_ID=你的飞书open_id
DEEPSEEK_API_KEY=你的DeepSeek_API_Key
```

重新打开已安装的工具箱后，只要应用仍在运行或隐藏在托盘中，手机私聊机器人即可正常工作。首次启动且尚未配置时，应用会在同一目录生成 `feishu-bridge.env.example` 作为模板；请复制为 `.env` 后填写。

机器人会将最近80条收发消息及尚未过期的结构化候选列表保存到本机；重启工具箱后会恢复其中最近48条作为DeepSeek的对话上下文。候选列表有效期为10分钟，过期后需要重新查询。

交付约定：每次完成并验证改动后，自动提交并推送至GitHub仓库；提交说明必须概括本次改动项。每次创建安装包时，默认更新现有的`E:\toolkit\personal-toolbox`安装目录（安装向导仍允许改选其他目录）。仅在明确要求发布，或功能达到一个完整版本时，才更新`package.json`版本号、推送标签并发布Release。GitHub Actions会自动构建安装包、生成更新元数据并创建GitHub Release：

```powershell
git tag v1.6.0
git push origin v1.6.0
```

本地安装包可通过`npm run dist`构建，结果位于`release/`目录；运行安装包时会默认更新`E:\toolkit\personal-toolbox`。若需要单文件便携版，可继续运行`npm run dist:portable`；便携版不支持自动更新。

## 数据存储

应用使用electron-store保存本地数据。默认目录为：

```text
%APPDATA%\personal-toolbox\
```

主要数据文件：

- `water-data.json`：饮水设置和历史记录
- `todo-data.json`：任务和标签
- `pomodoro-data.json`：专注任务、标签、活动计时和历史记录
- `finance-data.json`：账目和自定义标签
- `app-data.json`：关闭行为、快捷键、开机启动和操作确认设置

更新程序不会主动删除这些文件。记账数据可在应用内导出为JSON文件，建议定期备份。

## 项目结构

```text
toolkit/
├─ electron/
│  ├─ main.js                 Electron主进程、托盘、通知和本地存储
│  ├─ preload.js              渲染进程IPC接口
│  └─ assets/                 应用图标
├─ scripts/
│  ├─ build-portable.cjs      便携版构建脚本
│  └─ verify-finance-ui.cjs   界面与窗口尺寸回归检查
├─ src/
│  ├─ modules/
│  │  ├─ drinking/            饮水提醒
│  │  ├─ todo/                待办清单
│  │  ├─ finance/             记账助手
│  │  └─ PomodoroApp.jsx      番茄钟
│  ├─ pages/Home.jsx          工具箱主页
│  ├─ pages/AppSettings.jsx   工具箱基础设置
│  ├─ App.jsx                 页面路由
│  └─ styles.css              全局与模块样式
└─ package.json
```

## 窗口与退出行为

- 默认窗口大小为1280×820，最小窗口大小为960×640。
- 关闭窗口时可选择隐藏到托盘或彻底退出。
- 选择退出后，程序会停止提醒任务、销毁窗口和托盘并结束Electron进程。
- 可使用`Ctrl+Shift+X`切换主窗口：隐藏或最小化时恢复；显示时立即隐藏到托盘。

## 常见问题

### Windows提示应用来源未知

当前便携版未配置代码签名，Windows可能显示SmartScreen提示。请确认程序来自本仓库构建产物后再运行。

### 关闭窗口后仍有通知

这通常表示选择了“隐藏到托盘”。右键托盘图标并选择“退出”，或在“文件→设置”中修改默认关闭行为。

### 如何迁移记账数据

在记账助手的今日记账页面选择“导出JSON”，在另一台设备上选择“恢复JSON”即可。恢复操作会覆盖当前账目，建议先导出现有数据。
